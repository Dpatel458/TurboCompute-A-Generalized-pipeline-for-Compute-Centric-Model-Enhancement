...
def _create_efficiency_bars(path, title, base_params, model_params, base_time, model_time):
    """Horizontal bar chart for compression, speedup, and efficiency
       Works for Base vs Pruned OR Base vs Retrained
    """
    ...
    # ---- Calculate metrics ----
    compression_ratio = base_params / model_params if model_params > 0 else 1
    speedup = base_time / model_time if model_time > 0 else 1
    efficiency_score = (compression_ratio + speedup) / 2

    labels = ["Compression", "Speedup", "Efficiency"]
    values = [compression_ratio, speedup, efficiency_score]

    colors = [
        THEME_COLORS["pruned"],     # compression
        THEME_COLORS["accent"],     # speedup
        THEME_COLORS["retrained"]   # efficiency
    ]
    ...
    # ---- Axis formatting ----
    ax.set_yticks(y_pos)
    ax.set_yticklabels(labels, fontsize=13, color=THEME_COLORS["text"])
    ax.set_xlabel("Metric Value", fontsize=12, color=THEME_COLORS["text"])

    ax.tick_params(axis='x', colors=THEME_COLORS["text"])
    ax.grid(axis='x', linestyle='--', alpha=0.3, color=THEME_COLORS["grid"])

    # Remove borders
    for spine in ax.spines.values():
        spine.set_visible(False)

    plt.title(title, fontsize=18, color=THEME_COLORS["text"], fontweight="bold")
    plt.tight_layout()
...
def generate_graph(base_model_path, pruned_model_path, out_dir, run_id, index, flops_pair=None):
    ...
    if index == 1:
        ...
        _create_donut_comparison(path, "Model Size Comparison", size_b, size_p,
                                unit="GB", center_text=f"{reduction:.2f}% Reduction")
        return path, total

    # 2) GAUGE CHARTS - Efficiency Metrics
    if index == 2:
        path = os.path.join(run_dir, "efficiency_bars.png")
        params_b = _count_params(base)
        params_p = _count_params(pruned)
        time_b = _inference_time_ms(base)
        time_p = _inference_time_ms(pruned)

        _create_efficiency_bars(
            path,
            "Efficiency Metrics",
            params_b,
            params_p,
...
def generate_graph_retrain(base_model_path, pruned_model_path, retrained_model_path, out_dir, run_id, index):
    ...
    # 2) EFFICIENCY METRICS - Base, Pruned, Retrained
    if index == 2:
        path = os.path.join(run_dir, "efficiency_models.png")
